...
* @param {Object} results The object returned from the API.
*/
function printVisitsInfo(results) {
    $.each(results.rows[0], function (k, v) {
        output.push("<td>" + v + "</td>");
    });
}
...
function printReportInfo(results) {
    output.push(
      ...
      '<table class="infotable"><tbody><tr>',
      '<td width="180px">Contains Sampled Data :</td><td>', results.containsSampledData, '</td></tr><tr>',
      '<td>Kind                  :</td><td>', results.kind, '</td></tr><tr>',
      '<td>                      :</td><td>', results.id, '</td></tr><tr>',
      '<td>Self Link             :</td><td>', results.selfLink, '</td></tr><tr>',
      '</tbody></table>');
}
...
function outputToPage(output) {
    ...
    document.getElementById('idOfDynamic').innerHTML = output;
}
